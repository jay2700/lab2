...
# Calculating the number of unique artists represented in the collection
...
artist_missing_year_count <- the_tate|>dplyr::filter(is.na(year)) |> 
  ...
  arrange(desc(missing_year)) |> #Arranging the results in descending order of missing 'year' counts#
  as_tibble()
...
artist_missing_year_count <- artist_missing_year_count |> # Calculating the percentage of missing 'year' values for each artist #
  mutate(percentage = (missing_year / works_with_missing_year) * 100)

artist_missing_year_count <- artist_missing_year_count |>
  mutate(cumulative_percentage = cumsum(percentage)) # Calculating the cumulative percentage of missing 'year' values # 
...
```{r}
# Calculating the count of works for each artist in the 'the_tate' dataset #
artist_work_counts <- the_tate |>
  group_by(artist) |>
  summarize(title = n()) |>
  arrange(desc(title))

# Extracting and displaying the top 10 artists with the most works #
...
#Displaying the formatted table using print #
print(table)
```
...
# Selecting only the columns for artist and title to count distinct combinations #
...
# Counting and printing the number of duplicated artist-title pairs
...
selected_artworks <- the_tate |>
  ...
  drop_na() # Removing rows with containing NA values#
...
# Removing smallest artwork in the collection #
...
data <- data |> # Adding a column for the year of transaction #
...
summary_data <- data |> #summarising the data to group by year #
  group_by(Year) |>
  summarize(
    Annual_Return = (exp(sum(rd, na.rm = TRUE)) - 1)*100, 
    Annual_StdDev = sqrt(sum(vard, na.rm = TRUE))*100,
    .groups = "drop"  # Dropping grouping after summary
  )
...
# Calculating period return and period volatility
period_return <- prod(1 + summary_data$Annual_Return) - 1
period_volatility <- sqrt(sum(summary_data$Annual_StdDev^2))

# Creating summary rows for period return and period volatility
